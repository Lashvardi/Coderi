...
function registerHTMLSnippets(monaco: Monaco) {
  ...
  > = {
    ...
    div: {
      prefix: "div",
      body: '<div${1: class="$2"}>$0</div>',
      description: "div ელემენტი",
    },
    ...
    btn: {
      prefix: "btn",
      body: '<button${1: id="$2"}>$0</button>',
      description: "ღილაკი",
    },
    input: {
      prefix: "input",
      body: '<input type="${1:text}" ${2:placeholder="$3"} />',
      description: "ინპუტი",
    },
    ...
    "link:css": {
      ...
      description: "CSS ფაილის ჩართვა",
    },
    "script:src": {
      prefix: "script:src",
      body: '<script src="$1"></script>',
      description: "JS ფაილის ჩართვა",
    },
    section: {
      prefix: "section",
      body: '<section${1: class="$2"}>\n  $0\n</section>',
      description: "სექცია",
    },
    nav: {
      prefix: "nav",
      body: "<nav>\n  $0\n</nav>",
      description: "ნავიგაცია",
    },
    header: {
      prefix: "header",
      body: "<header>\n  $0\n</header>",
  ...
  monaco.languages.registerCompletionItemProvider("html", {
    ...
    provideCompletionItems: (
      ...
      // ვეძებთ prefix-ს კურსორამდე (ასოები, რიცხვები, !, :, >)
      const prefixMatch = textBefore.match(/([a-zA-Z0-9!:>]+)$/);
      const prefix = prefixMatch ? prefixMatch[1] : "";
      const startCol = prefixMatch
      ...
      const suggestions = Object.values(snippets)
        .filter((s) => s.prefix.startsWith(prefix) || prefix === "")
        .map((s) => ({
          label: s.prefix,
...
function registerCSSSnippets(monaco: Monaco) {
  ...
  monaco.languages.registerCompletionItemProvider("css", {
    provideCompletionItems: (
      ...
      const suggestions = Object.values(snippets).map((s) => ({
        label: s.prefix,
        kind: monaco.languages.CompletionItemKind.Snippet,
        documentation: s.description,
        insertText: s.body,
